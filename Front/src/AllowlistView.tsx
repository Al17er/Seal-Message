...
const Feeds: React.FC<{ suiAddress: string }> = ({ suiAddress }) => {
  ...
  const fetchFeedById = async (id: string): Promise<FeedData | null> => {
    ...
    try {
      ...
      if (allowlist.data?.content?.dataType !== 'moveObject') return null;

      const encryptedObjects = await suiClient
          .getDynamicFields({ parentId: id })
          .then((res) => res.data.map((obj) => obj.name.value as string));

      const fields = (allowlist.data.content as any).fields || {};
      return {
        allowlistId: id,
        allowlistName: fields.name || 'Unnamed',
  ...
  useEffect(() => {
    return () => {
      stopPolling();
      chatState.decryptedFiles.forEach((f) => {
        if (f.url.startsWith('blob:')) {
          URL.revokeObjectURL(f.url);
        }
      });
    };
  }, [chatState.decryptedFiles]);
