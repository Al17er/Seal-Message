...
module SealMessage::allowlist;

use std::string::String;
use sui::dynamic_field as df;
use SealMessage::utils::is_prefix;
...
public fun add(allowlist: &mut Allowlist, cap: &Cap, account: address) {
    assert!(cap.allowlist_id == object::id(allowlist), EInvalidCap); //assert 判断为False抛出错误
...
public fun publish(allowlist: &mut Allowlist, blob_id: String) {
    // assert!(cap.allowlist_id == object::id(allowlist), EInvalidCap);  cap: &Cap
